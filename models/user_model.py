...
class User:
    ...
    def __init__(
        self, user_id=None, name=None, email=None, password_hash=None, password_salt=None, user_type=UserType.Developer
    ):
        self.__id = user_id
        self.name = name
        self.email = email
    ...
    def __str__(self):
        return " Id: {0}\n User Name: {1}\n Email: {2}\n Password Hash: {3}\n Password Salt: {4}\n Type: {5}".format(
            self.__id, self.name, self.email, self.__passwordHash, self.__passwordSalt, self.type
        )
    ...
    def deserializable(cls, json_object):
        ...
        user_password_hash = (
            json_object["passwordHash"] if json_object is not None and "passwordHash" in json_object else None
        )
        user_password_salt = (
            json_object["passwordSalt"] if json_object is not None and "passwordSalt" in json_object else None
        )
    ...
    def post_instance(cls, json_object):
        ...
        user_name = json_object["name"] if json_object is not None and "name" in json_object else None
        user_email = json_object["email"] if json_object is not None and "email" in json_object else None
        user_password = json_object["password"] if json_object is not None and "password" in json_object else None
        user_type = json_object["type"] if json_object is not None and "type" in json_object else UserType.Developer
    ...
    def to_view_model(self):
        return (
            {"id": str(self.id), "name": self.name, "email": self.email, "type": self.type} if self is not None else {}
        )
