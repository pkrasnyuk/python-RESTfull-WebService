...
class UsersController:
    ...
    def add_user(self, data):
        try:
            user = models.User.post_instance(data)

            if user is not None and not self.__manager.check_user_email(user.email):
                return Response(
                    json.dumps({"message": "An user must have unique email"}), mimetype="application/json", status=422
                )

            status = self.__manager.add_user(user) if self.__manager is not None else False
            return (
                Response(
                    response=json.dumps({"message": "User added successfully"}), mimetype="application/json", status=201
                )
                if status
                else Response(json.dumps({"message": "No user added"}), mimetype="application/json", status=500)
            )
    ...
    def get_user(self, user_id):
        ...
        return (
            result.to_view_model()
            if result is not None
            else Response(response=json.dumps({"message": "User not found"}), mimetype="application/json", status=404)
        )
    ...
    def delete_user(self, user_id):
        ...
        if user is not None:
            ...
            return (
                Response(
                    response=json.dumps({"message": "User removed successfully"}),
                    mimetype="application/json",
                    status=200,
                )
                if result
                else Response(
                    response=json.dumps({"message": "No user removed"}), mimetype="application/json", status=500
                )
            )
        else:
    ...
    def update_user(self, user_id, data):
        try:
            user = self.__manager.get_user(user_id)
            if user is not None:
                user = models.User.update(user, data, {__import__})

                if user is not None and not self.__manager.check_user_email(user.email):
                    return Response(
                        json.dumps({"message": "An user must have unique email"}),
                        mimetype="application/json",
                        status=422,
                    )
                ...
                return (
                    ...
                    if result
                        ...
                        response=json.dumps({"message": "No user updated"}), mimetype="application/json", status=500
                    )
                )
            else:
                return Response(
                    response=json.dumps({"message": "User not found"}), mimetype="application/json", status=404
                )
        except ValueError as e:
            return Response(json.dumps({"message": e}), mimetype="application/json", status=422)
        except Exception:
    ...
    def login_user(self, data):
        if data is not None:
            ...
            if common.is_blank(user_email) or common.is_blank(user_password):
                return Response(
                    response=json.dumps({"message": "Incorrect request"}), mimetype="application/json", status=500
                )
            else:
                ...
                if user is None:
                    return Response(response=json.dumps({"message": message}), mimetype="application/json", status=404)
                else:
                    is_password_valid = security_helper.validate_password(
                        user_password, user.password_hash, user.password_salt
                    )
                    if is_password_valid:
                        user_token = security_service.generate_token(user)
                        return Response(
                            response=json.dumps({"token": "Bearer {0}".format(user_token.decode("utf-8"))}),
                            mimetype="application/json",
                            status=200,
                        )
                    else:
                        return Response(
                            response=json.dumps({"message": message}), mimetype="application/json", status=404
                        )
        else:
            return Response(
                response=json.dumps({"message": "Incorrect request"}), mimetype="application/json", status=500
